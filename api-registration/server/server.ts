...
app.post('/api/auth/sign-up', async (req, res, next) => {
  try {
    ...
    // delete user[hashedPassword]; // <- do this so the code wont return the hashed password OR include the properties you DO want returned in the returning statement in the sql as done on line 39.
    if (!user) {
      throw new ClientError(404, 'Username or Password failed. Try again');
    }
